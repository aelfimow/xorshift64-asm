...
try
{
    ...
    if (argc != 4)
    {
        throw std::invalid_argument("Usage: xorshift seed index total.");
    }
    ...
    if (index >= (sizeof(all_func) / sizeof(all_func[0])))
    {
    ...
    uint64_t state { seed };
    xorshift_func_t func = all_func[index];

    for (uint64_t cnt = 0; cnt < total; ++cnt)
    {
        state = func(state);
